...
export default function Lightbox({ images, selectedImage, setSelectedImage }) {
    ...
    return (
        <motion.div className={`fixed top-0 right-0 bottom-0 left-0 flex items-center justify-center z-[60] bg-black bg-opacity-50 text-white`}>
            ...
            <button className="absolute lg:top-1/2 top-[87%] lg:left-0 left-[25%] m-2" onClick={showPreviousImage}>
                Back
            </button>
            <button className="absolute lg:top-1/2 top-[87%] lg:right-0 right-[25%] m-2" onClick={showNextImage}>
                Forward
            </button>
        </motion.div>
    );
